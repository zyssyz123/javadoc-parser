...
def train(config):
    ...
    for i_e in range(num_iters):
        for tag, generator in train_gen.items():
            ...
            with open('./data/toy_example/ranking/losstest.txt', 'a') as fe:
                fe.write('%d\t%.6f\n' % (i_e, history.history['loss'][0]))
        ...
        for tag, generator in eval_gen.items():
            ...
            with open('./data/toy_example/ranking/MAPtest.txt', 'a') as fe:
                fe.write('%s\n' % (''.join(['%s\t%f\t'%(k,v/num_valid) for k, v in res.items()])))
            sys.stdout.flush()
